...
export default function RidesPage() {
  ...
  useEffect(() => {
    async function fetchRides() {
      const url =
        "https://ziptrip-ec0b6-default-rtdb.firebaseio.com/posts.json"; // Adjust URL for posts
  ...
  return (
    <section className="page">
      ...
      <h1 className="rides-h1">
        Rides from {fromLocation} to {toLocation} on {travelDate}
      </h1>

      {/* Display filtered rides */}
      <div className="grid">
        {filteredRides.length > 0 ? (
          filteredRides.map((ride) => <PostCard key={ride.id} ride={ride} />)
        ) : (
